ActiveAdmin.register Device do
  ...
  #Hide Delete Selected batch action
  ...
  batch_action :wipe, confirm: "Wipe the selected device(s)?", form: app_data do |ids,inputs|
    app = App.find(inputs["App Name"])
    batch = BatchInstallation.create(:app => app)
    ids.each do | id |
      install = Installation.new(device: Device.find(id),batch_installation: batch)
    ...
    redirect_to admin_dashboard_path, notice: "Successfully wiped device(s)"
  end
  
   batch_action :lock, confirm: "Lock the selected device(s)?",form: app_data do |ids,inputs|
   app = App.find(inputs["App Name"])
    batch = BatchInstallation.create(:app => app)
    ids.each do | id |
      install = Installation.new(device: Device.find(id),batch_installation: batch)
    ...
    redirect_to admin_dashboard_path, notice: "Successfully locked device(s)"
  end
